    ...
    IMcpAuthorisationService authorisationService) : McpToolBase<BoardListInput, BoardListOutput>(authorisationService)
{
    ...
    public override McpToolDefinition Definition { get; } =
        new(ToolNames.BoardList, "List boards accessible to the actor so clients can discover board ids before snapshot calls.", ToolSchemas.BoardListInput, ToolSchemas.ObjectOutput, MachinePatScopes.McpRead, ToolDiscoveryOrder.BoardList, McpToolBehaviours.ReadOnly);
