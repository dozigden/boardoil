...
- Board MCP operations:
  ...
  - when mentioning BoardOil cards in chat, format them as `<CardType emoji> #<card number>: <title> [<tag emoji><tag name>][<tag emoji><tag name>]...`
    - place the CardType emoji before the card number and title
    - leave one space after the title, then place each tag's emoji and name in its own square brackets (`[]`), with no whitespace inside the opening bracket, between the emoji and name, or between adjacent tags
    - use only emojis and names provided by BoardOil; do not infer or substitute them
    - omit a missing emoji while retaining the tag name, and omit all tag brackets when there are no tags
- Archive snapshots:
